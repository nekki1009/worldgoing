...
| Site definition | Rebuildable deterministic Site base, Site Travel Profile and explicit Region/Site entrance anchor | `SiteData`: stable ID, base version 2, Site Seed, landform, exit mask, local/global entrance meters and copied POI context | `WorldPOIData`, coordinates, deterministic hash | Runtime mutation, Scene nodes, UI | Implemented |
| Site generated layout | Fixed 50×50 Site grid at 2m per cell, deterministic bounds, presentation anchors and generated navigation flags | Rebuildable `SiteLayoutData`; no 2,500-cell mutable array or cached authority | SiteData, deterministic hash | Session, runtime state, Scene nodes, UI | Implemented; MOUNTAIN_PASS has generated NAV_BLOCKED cliffs, broader semantic Site content/collision is NOT IMPLEMENTED |
...
| Paper doll asset lab | Validate synthetic/approved visual catalogs, resolve detached preview recipes, preview one paper doll, and export deterministic contact sheets | `PaperDollPreviewDraft`, one fixed 11-Sprite Composer pool, transient facing/frame/timer state | Presentation visual Resources, Texture2D, Image, UI | GameSession mutation, Persistence, gameplay Item/Mount, Unit/Battle state | Implemented as Gate 0 / Milestone 1 synthetic Presentation slice; PC appearance and production art are NOT IMPLEMENTED |
...
- `SiteData` reuses `WorldPOIData.poi_id`, derives a versioned deterministic Site Seed, copies the lightweight strategic landform/exit profile, and defines Site-local meters through a reversible local/global entrance anchor at the center of the parent Strategic Cell. It does not contain mutable gameplay state.
- `SiteLayoutData` owns the fixed `50×50` grid and `2m` cell constants. `SiteLayoutGenerator` version 5 deterministically rebuilds the corresponding `100m×100m` bounds, layout anchors and generated navigation flags from `SiteData`; those bounds align exactly with the parent Strategic Cell, and `WorldData` exposes them without storing a second authoritative Site state.
- Every Strategic Cell can be queried lazily as one lightweight Site Travel Profile without allocating Site Runtime. `MOUNTAIN_PASS` is derived only for Mountain Road cells with at least two actual Route connections; its 1-byte eight-direction exit mask is shared by strategic travel and local pass layout. Region remains the future owner for generated Site resource/content summaries, while detailed Site content stays lazy.
...
- Road generation version 4 limits each deterministic terrain-aware Route to a 48-Cell corridor and only connects POIs inside the current finite `WorldData` bounds. POI candidate, road sample, graph, path and overlay caches remain rebuildable generated data, never Runtime authority.
...
The Site layer now has deterministic POI-backed base definitions, explicit local/global entrance anchors, a fixed 50×50 grid coordinate contract at 2m per cell, bounded/blocked WASD/arrow Party movement from `(25,25)`, generated local layout/navigation data, lazy Session-owned runtime state, detached snapshots, typed query/command results, and 36 focused architecture tests. MOUNTAIN_PASS collision is the only current semantic navigation slice; broader per-cell terrain/collision, gameplay content, movement time/animation and persistence remain intentionally outside this stage.
...
- `character_creator_test.gd`: 7/7 PASS for Catalog/ID/texture validation, gender and mount resolution, detached drafts, on-foot/mounted Recipe visibility, fixed 11-Sprite reuse, unified four-direction frame/flip/z-order control, pure Image contact sheets, full synthetic Catalog export, reusable UI lifecycle, lazy DebugUI entry, modal input restoration, and one-way Presentation dependencies.
- CharacterCreator visible runtime capture: PASS under project-default D3D12 Forward+ on NVIDIA GeForce RTX 5060; `.visual_captures/paper_doll/character_creator_milestone_1.png` was inspected for complete controls, tabs, preview, synthetic layers and Anchor guide without layout clipping.
...
- Existing tests: Coordinate 4/4, Terrain 9/9, POI 10/10, Road 13/13, Party movement 16/16, Cross-Region Travel 18/18, Region Seed + Delta 22/22 PASS. Runtime command/query also covers a shared cross-Region Preview Query. The Road suite includes an actual generated Mountain Road Site at Global Cell `(1832,38)` resolving to a directional pass.
...
Except for the MOUNTAIN_PASS `NAV_BLOCKED` corridor slice, semantic Site content, general per-cell terrain/collision, full Site pathfinding, movement time/animation, Site-local Party position persistence, Site Runtime persistence, active Travel persistence, and Save UI are NOT IMPLEMENTED. The generated Site layout does not yet define buildings, resources, NPCs or broader gameplay rules. Mountain terrain outside a pass remains slow but strategically passable, so world-scale impassable ridge topology is also NOT IMPLEMENTED. Region Construction currently supports only immediate Outpost place/remove with no cost or gameplay effect; roads, walls, farms, build time, workers and Site construction are NOT IMPLEMENTED. The current Battle Site is a deterministic read-only debug preview, not Combat. Exploration rules, settlement gameplay, economy, Character gameplay, PC appearance ownership/persistence, production paper-doll art, offline Battle variants/shader, combat runtime/resolution, NPC, AI, events, weather, quests, migrations, and multi-slot saves remain future work. The implemented synthetic paper-doll asset lab is Presentation tooling only and does not imply any of those owners exist. Their absence is not treated as a missing architecture skeleton when the current contract has a clear owner boundary.
