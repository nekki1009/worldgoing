...
| Paper doll asset lab | Validate visual catalogs, resolve detached split-part recipes, preview action clips, apply transient dyes, and export deterministic contact sheets | `PaperDollPreviewDraft`, action enum, one fixed 11-Sprite Composer pool, transient facing/frame/timer/dye state | Presentation visual Resources, Texture2D, Image, UI | GameSession mutation, Persistence, gameplay Item/Mount, Unit/Battle state | Split-part/action/dye lab implemented with 26 layer visuals, 2 mount bundles, and 2 presentation-only crafting recipes for light armor and its helmet; authored action sheets are optional and deterministic `PaperDollActionSheet` fallback preserves the 512x192 contract; dye masks are isolated (Hair+Brow, Armor+Barding, Cape, horse coat); PC appearance persistence, gameplay equipment/mount ownership, and Battle variants are NOT IMPLEMENTED |
...
variants of approved reference sheets; the raw generated board remains staged
because direct crops failed alignment.
