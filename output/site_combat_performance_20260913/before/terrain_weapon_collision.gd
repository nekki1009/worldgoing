...
func shield_shapes(actor: Variant) -> Array[PackedVector2Array]:
	...
	if skeleton == null:
		return result
	skeleton.force_update_all_bone_transforms()
	for node: Node in actor.editor.model_root.find_children("Shield_*", "MeshInstance3D", true, false):
		var mesh := node as MeshInstance3D
		if not mesh.is_visible_in_tree() or mesh.mesh == null:
			continue
		# A visible holstered shield protects only its actual silhouette too.
		var points := posed_points(actor, mesh, skeleton)
		if points.size() >= 3:
			result.append(Geometry2D.convex_hull(points))
	return result
...
static func contact(previous: Array[PackedVector2Array], current: Array[PackedVector2Array], bodies: Array[PackedVector2Array]) -> Dictionary:
	var best: Dictionary = {}
	for shape_index in range(current.size()):
		...
		var sweep_bounds := polygon_bounds(full_sweep).grow(0.001)
...
static func person_contact(previous: Array[PackedVector2Array], current: Array[PackedVector2Array], bodies: Array[PackedVector2Array], shields: Array[PackedVector2Array]) -> Dictionary:
	var body := contact(previous, current, bodies)
	var shield := contact(previous, current, shields)
...
func posed_points(actor: Variant, part: MeshInstance3D, skeleton: Skeleton3D) -> PackedVector2Array:
	var points := PackedVector2Array()
	var vertices := posed_vertices(part, skeleton)
...
func posed_vertices(part: MeshInstance3D, skeleton: Skeleton3D, native_rigid: bool = true) -> PackedVector3Array:
	...
	for surface in range(part.mesh.get_surface_count()):
		...
		if native_rigid and morphed == null:
			if palette.is_empty() or influences == 0:
			...
			if not _rigid_surfaces.has(key):
				...
				for vertex in range(vertices.size()):
					...
					if assigned < 0 or (rigid >= 0 and assigned != rigid):
						rigid = -2
						break
					rigid = assigned
				if _rigid_surfaces.size() > 128:
					_rigid_surfaces.clear()
				_rigid_surfaces[key] = rigid
			var rigid: int = _rigid_surfaces[key]
			if rigid >= 0:
				points.append_array(palette[rigid] * vertices)
				continue
