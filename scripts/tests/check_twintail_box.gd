...
func _run() -> void:
	...
	for hname in ["Hair_Long_01", "Hair_Long_02", "Hair_Long_03", "Hair_Long_04"]:
		...
		for v in verts:
			...
			# Test twintail box:
			if absf(hp.x) > 0.076 and hp.z > -0.090 and hp.z < -0.015 and hp.y > -0.050:
				count += 1
		print(hname, " in twintail box: count=", count)
	quit(0)
