...
func _run() -> void:
	...
	# Female with Leather Helmet on Auto
	...
	for h in [1, 2, 3, 4]:
		...
		await _settle(4)
		_save_capture("prod_f_h%d_front.png" % h)
		editor.set_preview_yaw_degrees(85.0)
		await _settle(4)
		_save_capture("prod_f_h%d_side.png" % h)
		editor.set_preview_yaw_degrees(175.0)
		await _settle(4)
		_save_capture("prod_f_h%d_back.png" % h)

	# Male with Leather Helmet on Auto
	...
	for h in [1, 2, 3, 4]:
		...
		editor.set_preview_yaw_degrees(0.0)
		await _settle(4)
		_save_capture("prod_m_h%d_front.png" % h)
		editor.set_preview_yaw_degrees(85.0)
		await _settle(4)
		_save_capture("prod_m_h%d_side.png" % h)
		editor.set_preview_yaw_degrees(175.0)
		await _settle(4)
		_save_capture("prod_m_h%d_back.png" % h)

	print("PROD_CAPTURES_DONE")
	editor.queue_free()
	await _settle(4)
	quit(0)
