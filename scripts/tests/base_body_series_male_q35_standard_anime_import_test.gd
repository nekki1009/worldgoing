...
const AssetPath: String = "res://assets/characters/human/q35/base_body_series_male_q35_standard_anime.glb"
...
func _run() -> void:
	...
	if not FileAccess.file_exists(absolute_path):
		_fail("Missing standard anime GLB: %s" % AssetPath)
		return
	var document := GLTFDocument.new()
	var state := GLTFState.new()
	var parse_error := document.append_from_file(absolute_path, state)
	if parse_error != OK:
		_fail("GLTFDocument could not parse standard anime GLB error=%d" % parse_error)
		return
	...
	if skeleton.get_bone_count() < 20:
		...
		return
	print("BASE_BODY_SERIES_MALE_Q35_STANDARD_ANIME_IMPORT_PASS: bones=%d meshes=%d" % [skeleton.get_bone_count(), mesh_count])
	instance.queue_free()
	quit(0)

func _fail(message: String) -> void:
	push_error("BASE_BODY_SERIES_MALE_Q35_STANDARD_ANIME_IMPORT_FAIL: " + message)
	quit(2)
