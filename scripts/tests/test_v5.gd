...
func _run() -> void:
	...
	var sh := Shader.new()
	sh.code = TEST_V5_SHADER

	# Male Hair 01 Side and 3/4
	...
	_apply_sh(editor, false, sh, 0.105, 0.136)

	editor.set_preview_yaw_degrees(85.0)
	await _settle(4)
	_save_capture("v5_m_h1_side.png")

	editor.set_preview_yaw_degrees(45.0)
	await _settle(4)
	_save_capture("v5_m_h1_34.png")

	editor.set_preview_yaw_degrees(0.0)
	await _settle(4)
	_save_capture("v5_m_h1_front.png")

	print("V5_CAPTURES_DONE")
	editor.queue_free()
	await _settle(4)
	quit(0)

func _apply_sh(ed: HumanCharacter3DEditor, is_female: bool, sh: Shader, brow_y: float, dome_y: float) -> void:
