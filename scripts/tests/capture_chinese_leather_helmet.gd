...
const DIR := "res://.visual_captures/chinese_leather_helmet"
...
func _run() -> void:
	...
	else: editor._load_body_model(index,"res://assets/characters/human/q35/chinese_leather_helmet/candidate_%s.glb" % sex)
	await settle(6)
	...
	for part in [[&"armor",&"armor_chinese_leather_01","Armor_Chinese_Leather_01_",&"armor_light_leather_01"],[&"helmet",&"helmet_chinese_leather_01","Helmet_Chinese_Leather_01_",&"helmet_steel_01"]]:
	...
	var sample := editor.model_root.find_child("Helmet_Chinese_Leather_01_DomePanel_0",true,false) as MeshInstance3D
	assert(sample.get_active_material(0).albedo_texture != null,"UV sample missing in engine")
	for choice in [&"helmet_leather_01",&"helmet_mingguang_01",&"helmet_chinese_leather_01",&"none",&"helmet_chinese_leather_01"]:
		assert(editor.select_part_by_id(&"helmet",choice))
		for pair in [[&"helmet_leather_01","Helmet_Leather_01"],[&"helmet_mingguang_01","Helmet_Mingguang_01_"],[&"helmet_chinese_leather_01","Helmet_Chinese_Leather_01_"]]:
			for node in editor.model_root.find_children(str(pair[1])+"*","MeshInstance3D",true,false):
				assert(node.visible == (choice==pair[0]),"Helmet switching residue")
	neutral()
	...
	frame_camera("head")
	var top_center := editor.camera.position+Vector3(0,0,5)
	editor.camera.position=top_center+Vector3(0,3,-.05)
	editor.camera.look_at(top_center)
	await capture(sex+"_helmet_top")
	editor.set_preview_yaw_degrees(0)
	frame_camera("chest")
	...
	for clip in clips:
		...
		for si in steps:
			...
			frame_camera("head")
			await capture("%s_head_%s_%02d" % [sex,clip,si])
		print("LEATHER_HELMET_CLIP ",sex," ",clip," ",steps)
	...
	file.close()
	print("LEATHER_HELMET_EDITOR_CAPTURE_PASS ",sex," ",mode)
	editor.queue_free()
	await settle(4)
	quit(0)
...
func combinations() -> void:
	...
	assert(editor.select_part_by_id(&"helmet",&"helmet_chinese_leather_01"))
...
func frame_camera(detail: String="") -> void:
	...
	if detail=="head":
		var sk := editor.model_root.find_child("Skeleton3D",true,false) as Skeleton3D
		center=sk.to_global(sk.get_bone_global_pose(sk.find_bone("J_Bip_C_Head")).origin)+Vector3(0,.055,0)
		editor.camera.size=.60
	elif detail=="chest":
