...
void fragment() {
	if (mask_enabled) {
		...
		// Forehead width (|X| <= 0.068), strictly in front of face (Z > 0.015), up to brow rim
...
func _run() -> void:
	...
	var sh := Shader.new()
	sh.code = TEST_V3_SHADER

	# Test Female Leather Helmet Hair 1-4
	...
	# For leather helmet: brow_y = 0.120 (dome vault bottom)
	for h in [1, 2, 3, 4]:
		...
		await _settle(4)
		_save_capture("v3_f_h%d_front.png" % h)
		editor.set_preview_yaw_degrees(85.0)
		await _settle(4)
		_save_capture("v3_f_h%d_side.png" % h)
		editor.set_preview_yaw_degrees(175.0)
		await _settle(4)
		_save_capture("v3_f_h%d_back.png" % h)

	# Test Male Leather Helmet Hair 1-4
	...
	for h in [1, 2, 3, 4]:
		...
		editor.set_preview_yaw_degrees(0.0)
		await _settle(4)
		_save_capture("v3_m_h%d_front.png" % h)
		editor.set_preview_yaw_degrees(85.0)
		await _settle(4)
		_save_capture("v3_m_h%d_side.png" % h)
		editor.set_preview_yaw_degrees(175.0)
		await _settle(4)
		_save_capture("v3_m_h%d_back.png" % h)

	print("V3_CAPTURES_DONE")
	editor.queue_free()
	await _settle(4)
	quit(0)
