...
const BASE := "res://output/site_army_loop_prepare_20260916/before/"
...
func _run() -> void:
	...
	assert(after_source.count(call) == 1)
	var restored := after_source.replace(call, "")
	for method: String in ["_animation_loop_default", "_prepare_animation_loop_defaults", "_apply_attack_loop_default"]:
		var matcher := RegEx.new()
		assert(matcher.compile("(?ms)^func " + method + "\\([^\\n]*\\n.*?(?=^func )") == OK)
		assert(matcher.search_all(restored).size() == 1)
		var old := matcher.search(before_source).get_string() if method == "_apply_attack_loop_default" else ""
		restored = restored.replace(matcher.search(restored).get_string(), old)
	assert(restored == before_source, "Only the verified loop rule extraction and load preparation changed")
	for relative: String in FILES:
		...
		assert(after.loop_prepare_revalidation.previous_editor_md5 == FileAccess.get_md5(old_editor) and after.loop_prepare_revalidation.rebaked == false)
		...
		after.erase("loop_prepare_revalidation")
		...
		assert(after == before, "Preserve all pixels/frame/rect/anchor/timing/appearance values: " + relative)
	...
	print("LOOP_PREPARE_ATLAS_PASS loop-load-only source; six provenance-only manifests; strict old/current/parent/mask hashes; three dyed loadouts; no rebake")
	quit(0)
