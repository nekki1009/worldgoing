...
const LOCAL_DIR: String = "res://.visual_captures/leather_test_preview"

var editor: HumanCharacter3DEditor

# Test Shader Candidate
const TEST_SHADER := """
shader_type spatial;
...
void fragment() {
	if (mask_enabled) {
		...
		// Front bang nose-bridge cull for helmets with nasal guards / brow center
		if (front_bangs_mode == 1 && head_local.z > 0.080 && head_local.y < 0.055 && abs(head_local.x) < 0.030) {
			discard;
		}

		// 1. Forehead Bangs Safe Zone:
		// Full forehead width (|X| <= 0.065), in front of skull (Z > 0.015), up to brow rim
		bool is_front_bangs = (head_local.z > 0.015 && head_local.y <= (brow_cut_y + 0.006) && abs(head_local.x) <= 0.065);

		if (!is_front_bangs) {
			// A. Top Cranial Dome & Spikes:
			// Any hair above the brow line inside the helmet dome ellipsoid or crown spikes poking out
			...
			// Cull ALL the way down (no lower Y bound) so zero floating severed stumps!
			if (abs(head_local.x) > 0.106 && head_local.z < 0.035) {
				discard;
			}

			// C. High Combat Ponytail (Female Hair 04):
			// High ponytail plume arches far behind skull (Z < -0.128)
...
func _run() -> void:
	...
	# Replace shader in editor
	var test_sh := Shader.new()
	test_sh.code = TEST_SHADER
	editor._hair_mask_shader = test_sh
	...
	for h in [&"hair_short_01", &"hair_short_02", &"hair_short_03", &"hair_short_04"]:
		...
		# Ensure override shader is our test shader and brow_y is correct
		_apply_test_params(editor, false, &"helmet_leather_01", test_sh)
		editor.set_preview_yaw_degrees(0.0) # Front
		await _settle(6)
		await _capture_dual("test_male_leather_%s_front.png" % h)
		editor.set_preview_yaw_degrees(35.0) # 3/4
		await _settle(6)
		await _capture_dual("test_male_leather_%s_three_quarter.png" % h)
		editor.set_preview_yaw_degrees(85.0) # Side
		await _settle(6)
		await _capture_dual("test_male_leather_%s_side.png" % h)
		editor.set_preview_yaw_degrees(175.0) # Back
		await _settle(6)
		await _capture_dual("test_male_leather_%s_back.png" % h)

	# Test Female with Leather Helmet
	...
	for h in [&"hair_short_01", &"hair_short_02", &"hair_short_03", &"hair_short_04"]:
		editor.select_part_by_id(&"hair", h)
		_apply_test_params(editor, true, &"helmet_leather_01", test_sh)
		editor.set_preview_yaw_degrees(0.0) # Front
		await _settle(6)
		await _capture_dual("test_female_leather_%s_front.png" % h)
		editor.set_preview_yaw_degrees(35.0) # 3/4
		await _settle(6)
		await _capture_dual("test_female_leather_%s_three_quarter.png" % h)
		editor.set_preview_yaw_degrees(85.0) # Side
		await _settle(6)
		await _capture_dual("test_female_leather_%s_side.png" % h)
		editor.set_preview_yaw_degrees(175.0) # Back
		await _settle(6)
		await _capture_dual("test_female_leather_%s_back.png" % h)

	print("TEST_CANDIDATE_RUN_COMPLETE")
	editor.queue_free()
	await _settle(4)
	quit(0)

func _apply_test_params(ed: HumanCharacter3DEditor, is_female: bool, helmet_id: StringName, sh: Shader) -> void:
	...
	var brow_y := 0.092 if not is_female else 0.088
	var bangs_mode := 1

	for node in ed.model_root.find_children("*", "MeshInstance3D", true, false):
		var mesh_node := node as MeshInstance3D
		if mesh_node.name.begins_with("Hair_"):
			for s in range(mesh_node.mesh.get_surface_count()):
				...
				if mat != null:
					...
					mat.set_shader_parameter("front_bangs_mode", bangs_mode)
