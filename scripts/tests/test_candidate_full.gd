...
void fragment() {
	if (mask_enabled) {
		...
		// Optional front bang culling for helmets:
		// Mode 2: Clean tuck (cull forehead hair under helmet coif)
		if (front_bangs_mode == 2 && head_local.z > 0.015 && head_local.y > 0.010) {
			discard;
		}
		...
		// Broad front coverage (|X| <= 0.068), strictly in front of face (Z > 0.015), up to brow rim
		bool is_front_bangs = (head_local.z > 0.015 && head_local.y <= (brow_cut_y + 0.008) && abs(head_local.x) <= 0.068);

		if (!is_front_bangs) {
			// A. Cranial Dome & Crown Spikes (culls hair hidden inside helmet dome):
			if (head_local.y > brow_cut_y) {
				vec3 d = (head_local.xyz - mask_center_offset) / mask_radii;
				if (dot(d, d) <= 1.06) {
					discard;
				}
				// Crown spikes / high knots poking through dome top:
			...
			// Twintails flare out laterally far beyond skull width.
			// Female skull hair max |X| is ~0.104; twintails reach 0.196.
			// Male head is wider (max |X| ~0.118) and has NO twintails.
			...
			// Ponytail plume arches far behind skull (Z < -0.128)
			// or hangs below nape (Y < -0.05 && Z < -0.105 && |X| < 0.060)
			// Flowing hair (Female Hair 02) stays within Z >= -0.127 and is 100% preserved!
...
func _run() -> void:
	...
	var sh := Shader.new()
	sh.code = TEST_CANDIDATE_SHADER

	# Female Leather Helmet Hair 1-4
	...
	for h in [1, 2, 3, 4]:
		...
		_apply_sh(editor, true, sh, 0.102)
		editor.set_preview_yaw_degrees(0.0)
		await _settle(4)
		_save_capture("cand_f_h%d_front.png" % h)
		editor.set_preview_yaw_degrees(85.0)
		await _settle(4)
		_save_capture("cand_f_h%d_side.png" % h)
		editor.set_preview_yaw_degrees(175.0)
		await _settle(4)
		_save_capture("cand_f_h%d_back.png" % h)

	# Male Leather Helmet Hair 1-4
	...
	for h in [1, 2, 3, 4]:
		...
		editor.set_preview_yaw_degrees(0.0)
		await _settle(4)
		_save_capture("cand_m_h%d_front.png" % h)
		editor.set_preview_yaw_degrees(85.0)
		await _settle(4)
		_save_capture("cand_m_h%d_side.png" % h)
		editor.set_preview_yaw_degrees(175.0)
		await _settle(4)
		_save_capture("cand_m_h%d_back.png" % h)

	print("CANDIDATE_CAPTURES_DONE")
	editor.queue_free()
	await _settle(4)
	quit(0)
