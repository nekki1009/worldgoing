...
func _run() -> void:
	print("--- Female Models: check (abs(X) > 0.085 && Z < 0.015) ---")
	_test_model(FEMALE_MODEL_PATH)
	print("--- Male Models: check (abs(X) > 0.095 && Z < 0.015) ---")
	_test_model(MALE_MODEL_PATH)
	quit(0)

func _test_model(path: String) -> void:
