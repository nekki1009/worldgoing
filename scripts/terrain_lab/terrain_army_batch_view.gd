...
func prepare_idle() -> void:
	...
	var packet: Array = kernel.call("render_idle", _army.combat_units, _army.cells, _army.moving_to,
		_army.facing, publications, _appearances, _descriptors, _idle_samples, _army._sprites, TerrainRenderer.CELL_PIXELS)
...
func submit_prepared(index: int) -> bool:
	if _native_mask.is_empty() or _native_mask[index] == 0: return false
	_groups_ready = false
...
func flush() -> void:
	...
	if _groups_ready:
		for offset in range(0, _group_spans.size(), 5):
			...
			_flush_run(row, _group_spans[offset + 1], _rows[row], _group_spans[offset + 2], _group_spans[offset + 3], _group_spans[offset + 4])
		return
...
func _flush_run(row: int, run: int, members: Array, start: int, stop: int, page_index: int) -> void:
	if page_index < 0:
	...
	var buffer: PackedFloat32Array = packet[0]
	var bounds: Rect2 = packet[1]
...
func _pack_run(members: Array, start: int, stop: int, map_scale: float) -> Array:
	...
	return _pack_run_gd(members, start, stop, map_scale)
...
func _describe(appearance: Dictionary) -> Dictionary:
	...
	if not _page_keys.has(key):
		...
		var idle_directions: Array = []
		for direction: String in ["up", "right", "down", "left"]:
			var idle_key := "combat_idle|" + direction
			if not lookup.has(idle_key):
				idle_directions.append([])
				continue
			var frames: Array = sequences[idle_key]
			...
			idle_directions.append([int(lookup[idle_key]), float(frames[0].duration),
				float(frames.back().sample_time) < float(frames[0].duration) - 0.000001, times, anchors])
		_idle_samples.append(idle_directions)
