...
const STAGE := "res://output/equipment_palette_policy_20260914"
var prepare_only := false
const BASE := "res://assets/characters/terrain_lab_army/standard_soldier"

func _initialize() -> void:
	var args := OS.get_cmdline_user_args()
	assert(args.size() in [1, 2] and args[0] in ["--base", "--ranged", "--masks"])
	...
	var pairs := {}
	if args[0] == "--base":
		var manifest: Dictionary = JSON.parse_string(FileAccess.get_file_as_string(STAGE + "/base/standard_soldier_atlas.json"))
		...
		var png := Image.load_from_file(STAGE + "/base/standard_soldier_atlas.png")
		var resource := load(STAGE + "/base/standard_soldier_atlas.res") as Texture2D
		assert(png.get_data() == resource.get_image().get_data())
		var collision := FileAccess.open_compressed(STAGE + "/base/standard_soldier_collision.bin", FileAccess.READ, FileAccess.COMPRESSION_ZSTD)
		...
		for filename: String in ["standard_soldier_atlas.png", "standard_soldier_atlas.res", "standard_soldier_atlas.json", "standard_soldier_collision.bin"]:
			pairs[STAGE + "/base/" + filename] = BASE + "/" + filename
	elif args[0] == "--ranged":
		for weapon: String in Reader.WEAPONS:
			var directory := STAGE + "/ranged/" + weapon + "/"
...
func _publish(pairs: Dictionary, phase: String) -> bool:
	var backup := STAGE + "/baseline/" + phase + "/"
	...
	for source: String in pairs:
		...
		assert(destination.begins_with(BASE + "/") and source.begins_with(STAGE + "/"))
